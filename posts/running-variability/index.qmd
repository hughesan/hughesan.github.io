...
There's seemingly no room in this model of training for the concept of sampling variability, which is to say, if you could run a race multiple times in parallel, how much variation would there be in your finish times simply due to randomness? If we understand and account for this uncertainty, we can hobble away from a race with a lot more information about our performance than singular training and target paces.  
...
Since we can't run races in parallel, there are two major considerations in studying repeated performance variability: the effect of fatigue if performances are close together in time, and the effect of age if performances are not so close together in time. Both apply.
